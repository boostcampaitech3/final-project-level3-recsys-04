...
from utils import get_data, ndcg, recall, hit_K
...
parser.add_argument('--dataset', type=str, default=os.getenv("DATA_PATH"))
...
if args.train:
    for epoch in range(args.n_epochs):
        ...
        print(f'epoch {epoch} | valid recal@10: {valid_scores[-1]:.4f} | ' +
    ...
    save_name = f"RecVAE_{np.round(final_scores[2], 4)}"
    with open(os.path.join(args.dataset, f'models/{save_name}.pt'), 'wb') as f:
        torch.save(model, f)

if args.inference:
    inference(args, device, save_name)
